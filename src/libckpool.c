...
/*
 * CashAddr is deliberately decoded here instead of being treated as bech32 or
 * Base58Check.  validateaddress() tells us whether the node accepts an address,
 * but it does not change the textual encoding passed to address_to_txn().
 * Feeding a CashAddr string to b58tobin() silently produces the wrong hash160
 * and therefore a valid-looking payout script for the wrong address.
 */
static const char cashaddr_charset[] = "qpzry9x8gf2tvdw0s3jn54khce6mua7l";

static uint64_t cashaddr_polymod(const uint8_t *values, const size_t len)
{
...
static int cashaddr_to_txn(char *out, const char *addr)
{
	uint8_t check[256], payload[128], decoded[65];
	const char *colon, *data;
	size_t prefix_len, data_len, check_len = 0, payload_len;
	uint32_t acc = 0;
	int bits = 0, decoded_len = 0;
	bool lower = false, upper = false;
	uint8_t version, type, size;
	size_t i;

	colon = strrchr(addr, ':');
	if (!colon || colon == addr || !colon[1])
		return 0;
	prefix_len = colon - addr;
	data = colon + 1;
	data_len = strlen(data);
	if (prefix_len + 1 + data_len > sizeof(check) || data_len <= 8 ||
	    data_len > sizeof(payload))
		return 0;

	/* CashAddr forbids mixed case. */
	for (i = 0; addr[i]; i++) {
	...
	for (i = 0; i < prefix_len; i++)
		check[check_len++] = ((uint8_t)addr[i] | 0x20) & 0x1f;
	check[check_len++] = 0;

	for (i = 0; i < data_len; i++) {
		const char ch = (data[i] >= 'A' && data[i] <= 'Z') ?
				data[i] + ('a' - 'A') : data[i];
		const char *p = strchr(cashaddr_charset, ch);

		if (!p)
			return 0;
		payload[i] = p - cashaddr_charset;
		check[check_len++] = payload[i];
	}
	if (cashaddr_polymod(check, check_len))
		return 0;

	/* The final eight 5-bit symbols are the 40-bit checksum. */
	...
	for (i = 0; i < payload_len; i++) {
		...
		while (bits >= 8) {
			...
			if (decoded_len >= (int)sizeof(decoded))
				return 0;
			decoded[decoded_len++] = (acc >> bits) & 0xff;
		}
	}
	/* CashAddr conversion pads with zero bits only. */
	...
	/* ckpool's payout script buffers support the standard 20-byte forms.
	 * Token-aware types 2/3 identify the same P2PKH/P2SH locking scripts. */
	if (size != 0 || decoded_len != 21)
		return 0;
...
int address_to_txn(char *p2h, const char *addr, const bool script, const bool segwit)
{
	...
	/* A colon identifies the explicit-prefix CashAddr form.  Never let one
	 * fall through to the Base58Check decoder. */
	if (strchr(addr, ':')) {
		len = cashaddr_to_txn(p2h, addr);
		if (!len)
			LOGWARNING("Failed to decode CashAddr payout address %s", addr);
		return len;
	}
	if (segwit)
